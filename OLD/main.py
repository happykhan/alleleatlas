...
from alleleatlas.core.dedup import deduplicate_at_exact_match, should_deduplicate
...
def main(cgmlst_profiles, outdir, target_nodes=None, force=False, nproc=4):
    ...
    # ========================================================================
    # STEP 1.5: Pre-collapse at exact match (HC0) for large datasets
    # ========================================================================
    # For datasets with many exact duplicates, deduplicate before distance
    # computation to achieve massive speedup (10x+ on high-duplicate datasets)
    if should_deduplicate(input_df.shape[0]):
        input_df, n_removed, st_counts = deduplicate_at_exact_match(input_df, st_counts)
        if n_removed > 0:
            n_reduced = input_df.shape[0]
            console.print(f'  Input profiles reduced to {n_reduced:,} for distance computation')
            console.print('  [dim]Will expand results back after clustering[/dim]')
